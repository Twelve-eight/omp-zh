...
const V = load('G:/omp works/Tools/omp-zh/work/omp-dl-1826.exe');
...
const checks = [
  ...
  ['stopcap cluster=1e6', has('iIn = 1000000, uqa = 4000, aIn = 1000000, lIn = 1000000')],
  ['session cap=1e6', has('qIn = 1000000')],
  ['yield ladder=1e6', has('dbt = 1000000')],
  ['retryRecovery natively rendered (wW)', has('function wW(e, t = 0)')],
  ['flush gate removed', !has('if (this.#t.size === 0 && this.#e.size === 0)')],
  ['flush x2 unconditional', (d.match(/for \(const t of e\)\n      this\.#R\(t\);\n    this\.#x\(\);/g) || []).length === 2],
  ['leak tunnel cwd+hide', has('cwd: Q6(), windowsHide: true })')],
  ['leak ssh cwd+hide', has('cwd: NIt.homedir(), windowsHide: true })')],
  ...
  ['console probe forced true', has('function Zts(e) {\n  if (e.platform !== "win32")\n    return false;\n  return true;\n}')],
